...
from KT_Cloud.DB.NEW_KT_DB.Exception.exception import DBInstanceNotFoundError, ParamValidationError
from KT_Cloud.DB.NEW_KT_DB.Validation.DBInstanceValidition import check_extra_params, check_required_params,is_valid_db_instance_identifier
from NEW_KT_DB.Models.DBInstanceModel import DBInstance
...
class DBInstanceService(DBO):
    ...
    def create(self, **attributes):
        ...
        if not is_valid_db_instance_identifier(db_instance_identifier):
        ...
        self.dal.createInMemoryDBInstance(db_instance.to_dict())
    ...
    def describe(self,db_instance_identifier):
        ...
        if not self.dal.is_db_instance_identifier_exist(db_instance_identifier):  # check if db to delete exists
            raise DBInstanceNotFoundError('This DB instance identifier does not exist')
        return {'DBInstance': self.dal.describeDBInstance(db_instance_identifier)}
    ...
    def modify(self, **updates):
        ...
        for param in all_params:
            update_db_instance[param]=updates[param]
