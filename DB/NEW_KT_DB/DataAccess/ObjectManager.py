...
class ObjectManager:
    ...
    def get_object_from_management_table(self, table_name, object_id: int, columns=["*"]) -> Dict[str, Any]:
        ...
        print('select_and_return_records_from_table:',result)
        if result:
    ...
    def get_objects_from_management_table_by_criteria(self,table_name, columns=["*"], criteria: Optional[str] = None) -> Dict:
        ...
        if result:
            ...
            raise FileNotFoundError(
                f'Objects with criteria {criteria} not found.')
    ...
    def save_in_memory(self,table_name, object):
        ...
        if not self.is_management_table_exist(table_name):
            self.create_management_table(table_name)
        self.insert_object_to_management_table(
            table_name, object.to_sql())
    ...
    def get_from_memory(self, object_name, columns=["*"], object_id=None, criteria=None):
        ...
        if object_id:
            criteria = f'object_id = {object_id}'
        return self.get_objects_from_management_table_by_criteria(
