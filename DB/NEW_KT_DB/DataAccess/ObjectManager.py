...
class ObjectManager:
    ...
    def create_management_table(
        self,
        table_name,
        table_structure="object_id INTEGER PRIMARY KEY AUTOINCREMENT,type_object TEXT NOT NULL,metadata TEXT NOT NULL",
    ):
        self.db_manager.create_table(table_name, table_structure)

    def create_management_table_with_str_id(
        self,
        table_name,
        table_structure="object_id TEXT NOT NULL PRIMARY KEY ,metadata TEXT NOT NULL",
    ):
        self.db_manager.create_table(table_name, table_structure)
    ...
    def insert_object_to_management_table(self, table_name, object_metadata):
        self.db_manager.insert_data_into_table(
            table_name, ["metadata"], (object_metadata)
        )
    ...
    def update_object_in_management_table_by_id(self, table_name, object_id, updates):
        self.db_manager.update_records_in_table(
            table_name, updates, f"object_id = {object_id}"
        )

    # rachel-8511, Riki7649255
    def get_object_from_management_table(
        self, table_name, object_id: int, columns=["*"]
    ) -> Dict[str, Any]:
    ...
    def get_objects_from_management_table_by_criteria(
        self, object_id: int, columns=["*"], criteria: Optional[str] = None
    ) -> Dict:
        """Retrieve an object from the database."""
        result = self.db_manager.select_and_return_records_from_table(
            self.table_name, columns, criteria
        )
        if result:
            return result
        else:
    ...
    def delete_object_from_management_table_by_id(self, table_name, object_id) -> None:
        """Delete an object from the database."""
        self.db_manager.delete_data_from_table(
            table_name, criteria=f"object_id = {object_id}"
        )
    ...
    # for outer use:
    def save_in_memory(
        self, object_name: str, metadata: Dict[str:Any], object_id: Optional[str] = None
    ):
        ...
        if object_id:
            self.insert_object_to_management_table_with_str_id(
                table_name, object_id, metadata
            )
    ...
    def delete_from_memory(
        self, object_name: str, criteria="default", object_id: Optional[str] = None
    ):
    ...
    def update_in_memory(
        self, object_name, updates, criteria="default", object_id: Optional[str] = None
    ):
    ...
    def get_from_memory(
        self, object_name, columns=["*"], object_id=None, criteria=None
    ):
        """get records from memory by criteria or id"""
        table_name = self.convert_object_name_to_management_table_name(object_name)
        if object_id:
            criteria = f"object_id = {object_id}"
        self.get_objects_from_management_table_by_criteria(
            table_name, columns, criteria
        )
