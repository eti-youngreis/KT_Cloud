...
class ObjectManager:
    ...
    def create_management_table(self, object_name, table_structure='default', pk_column_data_type='INTEGER'):
        ...
        table_name = self._convert_object_name_to_management_table_name(
            object_name)
        pk_constraint = ' AUTOINCREMENT' if pk_column_data_type == 'INTEGER' else ''

        if table_structure == 'default':
            table_structure = f'object_id {pk_column_data_type} PRIMARY KEY {
                pk_constraint},type_object TEXT NOT NULL,metadata TEXT NOT NULL'
    ...
    def _insert_object_to_management_table(self, table_name, object_info, columns_to_populate=None):
        ...
        if columns_to_populate is None:
            ...
            self.db_manager.insert_data_into_table(
                table_name, object_info, columns_to_populate)
    ...
    def save_in_memory(self, object_name, object_info, columns=None):
        ...
        if columns is None:
            ...
            self._insert_object_to_management_table(
                table_name, object_info, columns)
    ...
    def update_in_memory(self, object_name, updates, criteria):
        ...
        table_name = self._convert_object_name_to_management_table_name(
            object_name)
        self._update_object_in_management_table_by_criteria(
    ...
    def is_exists(self, object):
        ...
        try:
            query = f'select * from {table_name} where {
                object.pk_column} = {object.pk_value}'
