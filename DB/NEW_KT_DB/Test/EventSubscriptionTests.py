...
def test_describe(event_subscription_service: EventSubscriptionService, event_subscription: EventSubscription):
    assert is_object_equal(event_subscription, event_subscription_service.describe(
        f'{event_subscription.pk_column} = "{event_subscription.pk_value}"')[0])


def test_get(event_subscription_service: EventSubscriptionService, event_subscription: EventSubscription):
    assert is_object_equal(event_subscription, event_subscription_service.get(
        f'{event_subscription.pk_column} = "{event_subscription.pk_value}"')[0])
