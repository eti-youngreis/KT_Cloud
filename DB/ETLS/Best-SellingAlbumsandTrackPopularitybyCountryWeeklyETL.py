...
#         # ------------------------------------------------------
#         # Join Invoices, InvoiceLines, Tracks, Albums, and Customers
#         # ['InvoiceLineId', 'InvoiceId', 'TrackId', 'UnitPrice'] 
#         # ["TrackId"],InvoiceLines["UnitPrice"],InvoiceLines["AlbumId"],
#                                 #   InvoiceLines["InvoiceId"],InvoiceLines["CustomerId"]
#         # joined_data = InvoiceLines.join(Tracks, "TrackId") \
#         #                           .join(Albums, "AlbumId") \
#         #                           .join(Invoices, "InvoiceId") \
#         #                           .join(Customers, "CustomerId").drop(InvoiceLines["TrackId"],InvoiceLines["UnitPrice"],
#         #                             InvoiceLines["InvoiceId"],InvoiceLines["CustomerId"])
...
# #             F.col("a.Title")
# #         )
# #                 # Assuming 'invoice_lines_tracks_albums_df' is the result of your previous joins between InvoiceLines, Tracks, and Albums
# #         tracks_albums_invoices_df = tracks_albums_df.alias("tai").join(
# #         Invoices.alias("i"),
# #         on=F.col("tai.InvoiceId") == F.col("i.InvoiceId"),
# #         how="inner"
# #     ).select(
# #         F.col("tai.InvoiceId"),
# #         F.col("tai.TrackId"),
# #         F.col("tai.TrackUnitPrice"),
# #         F.col("tai.Quantity"),
# #         F.col("tai.Title"),
# #         F.col("i.CustomerId"),
# #     )

# # # Now you have joined InvoiceLines, Tracks, Albums, and Invoices


# #         invoice_customer_df=tracks_albums_df.alias("ic").join(
# #         Customers.alias("c"),
# #         on=F.col("ic.CustomerId") == F.col("c.CustomerId"),
# #         how="inner"
# #         ).select(
# #             F.col("c.CustomerId"),
# #             F.col("c.Country"),
...
# #         )
#         grouped_data = invoice_lines_tracks_df.groupBy("Country","AlbumId") \
#             .agg(
...
#         ranked_tracks = grouped_data.withColumn(
...
#         # grouped_data = invoice_lines_tracks_df.groupBy("Country ","AlbumId",  "TrackId") \
#         #     .agg(
#         #         F.sum("Quantity").alias("total_play_count"),
#         #         F.sum(F.col("Quantity") * F.col("UnitPrice")).alias("total_revenue")
#         #     )

#         # # Join back with Tracks to get Track information
#         # track_data = grouped_data.join(Tracks, "AlbumId", "inner")

#         # # Use window function to rank tracks within each album by popularity
#         # window_spec = Window.partitionBy("AlbumId", "Country").orderBy(F.desc("total_play_count"))
#         # ranked_data = track_data.withColumn("track_rank", F.rank().over(window_spec))

#         # Add metadata columns
#         current_datetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#         user_name = "tamar_koledetzky"  # Replace this with a method to fetch the current user's name if needed

#         final_data_with_metadata = ranked_tracks.withColumn("created_at", F.lit(current_datetime)) \
...
#             final_data_df.to_sql('Best_Selling_Albums_And_Track_Popularity_By_Country', conn, if_exists='replace', index=False)
