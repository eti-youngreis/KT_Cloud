...
def test_can_stop_DBCluster(db_setup, create_DBCluster):
    assert db_setup.StopDBCluster(db_cluster_identifier = "my-cluster1")

def test_handle_db_cluster_identifier_cannot_be_found(db_setup, create_DBCluster):
    with pytest.raises(ValueError):
        db_setup.StopDBCluster(db_cluster_identifier = "invalid-name")