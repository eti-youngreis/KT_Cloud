...
class EventManager:
    ...
    def update(self, event_id: int, metadata: Dict[str, Any]) -> None:
        self.db_manager.update(
            self.table_name, {'metadata': json.dumps(metadata)}, f'event_id = {event_id}')

    def get(self, event_id: int) -> Dict[str, Any]:
        result = self.db_manager.select(
            self.table_name, ['event_type', 'metadata'], f'event_id = {event_id}')
        if result:
    ...
    def get_all_events(self) -> Dict[int, Dict[str, Any]]:
        results = self.db_manager.select(
            self.table_name, ['event_id', 'event_type', 'metadata'])
