...
class PyStorage:
    ...
    async def put_object_tagging(self, file_path, tags, version_id=None, async_mode=False):
        # Check if async_mode is a boolean
        if not isinstance(async_mode, bool):
            raise TypeError("async_mode must be a boolean")

        # Update metadata tags asynchronously or synchronously based on async_mode
        if async_mode:
    ...
    async def get_object_tagging(self, file_path, version_id=None, async_mode=False):
        # Check if async_mode is a boolean
        if not isinstance(async_mode, bool):
            raise TypeError("async_mode must be a boolean")
