...
def test_get_object_tagging_file_with_tags(create_test_file, setup_metadata_manager):
    ...
    asyncio.run(functions.put_object_tagging(file_path, tags, version_id=456, async_mode=False))

    tags_sync = asyncio.run(functions.get_object_tagging(file_path, version_id=456, async_mode=False))
    tags_async = asyncio.run(functions.get_object_tagging(file_path, version_id=456, async_mode=True))
...
def test_get_object_tagging_file_without_tags(create_test_file, setup_metadata_manager):
    ...
    tags_sync = asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode=False))
    tags_async = asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode=True))
...
def test_get_object_tagging_empty_values_sync(create_test_file, setup_metadata_manager):
    ...
    result_sync = asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode=False))
    result_async = asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode=True))
...
def test_get_object_tagging_invalid_arguments(create_test_file, setup_metadata_manager):
    # Test that invalid arguments for `async_mode` raise a TypeError
    file_path = str(create_test_file)
    with pytest.raises(TypeError):
        asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode="a"))
        asyncio.run(functions.get_object_tagging(file_path, version_id=123, async_mode="m"))
...
def test_get_object_tagging_no_version_id(create_test_file, setup_metadata_manager):
    ...
    result_sync = asyncio.run(functions.get_object_tagging(file_path, async_mode=False))

    latest_version = setup_metadata_manager.get_latest_version(file_path)

    result = asyncio.run(functions.get_object_tagging(file_path,latest_version, async_mode=False))
