...
async def test_get_object_lock_configuration_non_existing_bucket(create_test_metadata):
    s3_client = create_test_metadata
    with pytest.raises(FileNotFoundError, match="Bucket nonexistent_bucket not found."):
        await s3_client.get_object_lock_configuration('nonexistent_bucket')


@pytest.mark.asyncio
async def test_get_object_lock_configuration_not_a_bucket(create_test_metadata):
    # Test with an invalid input that is not a bucket
    with pytest.raises(FileNotFoundError, match="Bucket invalid_input not found."):
        await create_test_metadata.get_object_lock_configuration('invalid_input')
...
async def test_put_object_valid(create_test_metadata):
    ...
    bucket_path = Path(s3_client.server_path +"/"+ 'bucket1')
