...
def test_put_object_tagging(create_test_metadata):
    ...
    bucket = 'bucket1'
    key = 'object1.txt'
    tags = [{'key': '111', 'value': 'Te22st'}]
    print(f"Metadata file path: {functions.metadata_manager.metadata }")

    asyncio.run(functions.put_object_tagging(bucket, key, tags, version_id='2', async_flag=False))
    asyncio.run(functions.put_object_tagging(bucket, key, tags, version_id='2', async_flag=True))

    metadata = functions.metadata_manager.metadata
    assert metadata['server']['buckets'][bucket]['objects'][key]['versions']['2']['TagSet'] == tags
...
def test_put_object_tagging_invalid_arguments(create_test_metadata):
    ...
    bucket = 'bucket1'
    key = 'object1.txt'
    tags = [{'key': 'Invalid', 'value': 'Arguments'}]

    with pytest.raises(TypeError):
        asyncio.run(functions.put_object_tagging(bucket, key, tags, version_id='2', async_flag='a'))
        asyncio.run(functions.put_object_tagging(bucket, key, tags, version_id='2', async_flag='m'))
...
def test_put_object_tagging_large_metadata(create_test_metadata):
    ...
    asyncio.run(functions.put_object_tagging(bucket, key, large_tags, version_id='2', async_flag=False))
    asyncio.run(functions.put_object_tagging(bucket, key, large_tags, version_id='2', async_flag=True))
...
def test_put_object_tagging_special_characters(create_test_metadata):
    ...
    bucket = 'bucket1'
    key = 'object1.txt'
    tags = [{'key': 'Key#1', 'value': 'Value@1'}, {'key': 'Key&2', 'value': 'Value*2'}]
