...
def load():
    ...
    try:
        cursor = conn.cursor()
        latest_timestamp_query = "SELECT MAX(updated_at) FROM employee_customer_satisfaction"
        ...
        new_invoice_lines = invoice_lines.filter(invoice_lines["updated_at"] > latest_timestamp)
