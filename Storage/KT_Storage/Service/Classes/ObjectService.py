...
class ObjectService:
    ...
    async def get_object(
        self, bucket, key, version_id: str = None, flag_sync: bool = True
    ):
        try:
            if (
                bucket not in self.object_manager.get_buckets()
                or key not in self.object_manager.get_bucket(bucket)["objects"]
            ):
    ...
    async def put_object(
        ...
        try:
            ...
            if not self.object_manager.get_bucket(bucket):
                ...
                self.object_manager.metadata["server"]["buckets"][
                    bucket
                ] = bucket_metadata
                object_metadata = {"versions": {}}
            else:
                object_metadata = self.object_manager.get_bucket(bucket)["objects"].get(
                    key, {"versions": {}}
                )
            ...
            await self.object_manager.create(
                bucket, key, data, body, version_id, object_metadata, flag_sync
            )
        except FileNotFoundError as e:
            print(f"Error: The file or directory was not found: {e}")
            raise
    ...
    async def copy_object(
        ...
        if version_status == "enabled" and version_id:
            await self.object_manager.copy_object(
                source_bucket,
                source_key,
                destination_bucket,
                destination_key,
    ...
    async def put_object_tagging(
        self, tags: Tag, bucket, key, version_id=None, sync_flag=True
    ):
        ...
        if version_id not in versions:
            raise FileNotFoundError(
                f"Version '{version_id}' does not exist in object '{key}'"
            )
        ...
        return {"VersionId": version_id, "Tag_Set": tags_dict}

    async def get_object_tagging(
        self, obj: ObjectModel, version_id=None, sync_flag=True
    ):
        # Validate sync_flag
        if not isinstance(sync_flag, bool):
            raise TypeError("sync_flag must be a boolean")
        ...
        if sync_flag:
            ...
            bucket_metadata = await asyncio.to_thread(
                self.object_manager.get_bucket, bucket
            )
        ...
        if version_id is not None:
            ...
            if version_id_str in versions:
                ...
                raise FileNotFoundError(
                    f"Version ID '{version_id}' does not exist for object '{key}' in bucket '{bucket}'"
                )
        else:
    ...
    async def put_object_acl(
        self, obj: ObjectModel, acl: Acl, version_id=None, is_sync=True
    ):
        bucket = obj.bucket
        key = obj.key
        # Check if the bucket exists
        ...
        return {
            "VersionId": version_id,
            "Acl": {"owner": acl.owner, "permissions": acl.permissions},
        }
    ...
    async def head_object(self, obj: ObjectModel, version_id=None, flag_async=True):
        ...
        try:
            ...
            if version_id not in versions:
                raise FileNotFoundError(
                    f"Version '{version_id}' does not exist for key '{key}' in bucket '{bucket}'."
                )
            ...
            return {
                ...
                "ContentType": version_metadata.get(
    ...
    async def get_object_attributes(
        self, obj: ObjectModel, version_id=None, MaxParts=None, sync_flag=False
    ):
        bucket = obj.bucket
        key = obj.key
        ...
        try:
            ...
            if sync_flag:
                ...
                metadata = await asyncio.to_thread(
                    self.object_manager.get_object, bucket, key
                )
    ...
    async def put_object_lock_configuration(
        ...
        try:
            ...
            metadata = self.object_manager.get_bucket(bucket)
            if not metadata:
