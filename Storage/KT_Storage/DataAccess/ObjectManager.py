...
class ObjectManager:
    ...
    async def create(
        self, bucket, key, data, body, version_id, object_metadata, sync_flag=True
    ):
    ...
    def get_bucket(self, bucket):
        ...
        bucket = Bucket(
            bucketJson["name"],
            bucketJson["object"],
            bucketJson["creationDate"],
            bucketJson["policy"],
            bucketJson["acl"],
            bucketJson["Tag"],
            bucketJson["cors_configuration"],
        )
        return bucket
    ...
    async def copy_object(
        ...
        if not source_metadata:
            raise FileNotFoundError(
                f"Source object {source_bucket}/{source_key} not found"
            )
        ...
        self.storage_maneger.copy(
            source_bucket, source_key, destination_metadata, sync_flag=sync_flag
        )
