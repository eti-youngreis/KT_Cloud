...
class MetadataManager:
    ...
    def get_latest_version(self, bucket, key):
        ...
        if metadata and "versions" in metadata:
            ...
            raise FileNotFoundError(
                f"No versions found for object {key} in bucket {bucket}"
            )
    ...
    async def update_metadata(self, bucket, key, version_id, data, is_sync=True):
        ...
        if key not in self.metadata["server"]["buckets"][bucket]["objects"]:
            self.metadata["server"]["buckets"][bucket]["objects"][key] = {
                "versions": {}
            }
    ...
    async def delete_version(self, bucket, key, version_id, is_sync=True):
        ...
        if (
            key in bucket_data.get("objects", {})
            and version_id in bucket_data["objects"][key]["versions"]
        ):
            del self.metadata["server"]["buckets"][bucket]["objects"][key]["versions"][
                version_id
            ]
            if not self.metadata["server"]["buckets"][bucket]["objects"][key][
                "versions"
            ]:
    ...
    async def delete_object(self, bucket, key, is_sync=True):
        if (
            bucket in self.metadata["server"]["buckets"]
            and key in self.metadata["server"]["buckets"][bucket]["objects"]
        ):
            del self.metadata["server"]["buckets"][bucket]["objects"][key]
            if is_sync:
                await self.save_metadata(True)
    ...
    async def check_permissions(
        self, bucket, key, version_id, by_pass_governance_retention, is_sync=True
    ):
        metadata = self.get_bucket_metadata(bucket, key)
        if metadata and version_id in metadata["versions"]:
            ...
            retain_until_date = version_metadata.get("retention", {}).get(
