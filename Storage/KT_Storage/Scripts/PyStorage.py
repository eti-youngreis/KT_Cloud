...
class S3ClientSimulator:
    ...
    async def get_object_attributes(
        ...
        if sync_flag:
            ...
            metadata = await asyncio.to_thread(
                self.metadata_manager.get_bucket_metadata, bucket, key
            )
        ...
        if version_metadata is None:
            raise FileNotFoundError(
                f"No version found with ID {version_id} for object {key}"
            )
    ...
    async def get_object_lock_configuration(self, bucket):
        # Check if the bucket exists
        bucket_metadata = (
            self.metadata_manager.metadata.get("server", {})
            .get("buckets", {})
            .get(bucket)
        )
        ...
        object_lock_configuration = {
            ...
            "LockConfiguration": (
                object_lock["lockConfiguration"] if object_lock else {}
            ),
        }

        return {"ObjectLockConfiguration": object_lock_configuration}
    ...
    async def get_object_torrent(
        ...
        range=None,
        ssec_ustomer_algorithm=None,
        ssec_ustomer_key=None,
        ssec_ustomerkey_md5=None,
        request_payer=None,
    ):
        ...
        if version_id:
            ...
            if not version_metadata:
                raise FileNotFoundError(
                    f"Version {version_id} not found for object {key} in bucket {bucket}"
                )
                raise FileNotFoundError(
                    f"Version {version_id} not found for object {key} in bucket {bucket}"
                )
    ...
    async def head_object(
        ...
        if not version_metadata:
            raise FileNotFoundError(
                f"No version metadata found for object {key} with version {version_id}"
            )
    ...
    async def put_object(
        ...
        if not bucket_metadata:
            ...
            self.metadata_manager.metadata["server"]["buckets"][
                bucket
            ] = bucket_metadata
        ...
        object_metadata["versions"][version_id] = {
            ...
            "acl": (
                acl
                if acl
                else {"owner": "default_owner", "permissions": ["READ", "WRITE"]}
            ),
    ...
    async def put_object_acl(
        ...
        # Check if the bucket exists
        bucket_metadata = self.metadata_manager.metadata["server"]["buckets"].get(
            bucket
        )
        if not bucket_metadata:
            raise FileNotFoundError(f"Bucket '{bucket}' does not exist")
        ...
        if not version_id or version_id not in object_metadata["versions"]:
            raise FileNotFoundError(
                f"Version '{version_id}' does not exist for object '{key}' in bucket '{bucket}'"
            )
        ...
        return {"VersionId": version_id, "ACL": acl}
    ...
    async def copy_object(self, bucket_name, copy_source, key, is_sync=True):
        ...
        try:
            ...
            print(source_key)
            await self.metadata_manager.copy_metadata(
                source_bucket, source_key, bucket_name, key, is_sync=is_sync
            )
            ...
            if not source_file_path.exists():
                raise FileNotFoundError(
                    f"Source file {source_file_path} not found {source_file_path}"
                )
            ...
            destination_metadata = self.metadata_manager.get_bucket_metadata(
                bucket_name, key
            )
            latest_version = self.metadata_manager.get_latest_version(bucket_name, key)
            destination_version_metadata = destination_metadata["versions"][
                latest_version
            ]
    ...
    async def delete_object(self, bucket_name, key, is_sync=True):
        delete_result = await self.metadata_manager.delete_object(
            bucket_name, key, is_sync=is_sync
        )
        if delete_result:
    ...
    async def delete_objects(self, bucket_name, delete, is_sync=True):
        ...
        for obj in delete["Objects"]:
            ...
            try:
                if version_id:
                    delete_result = await self.metadata_manager.delete_version(
                        bucket_name, key, version_id, is_sync=is_sync
                    )
                else:
                    delete_result = await self.metadata_manager.delete_object(
                        bucket_name, key, is_sync=is_sync
    ...
    async def get_object_acl(self, bucket_name, key, is_sync=True):
        try:
            latest_version = self.metadata_manager.get_latest_version(bucket_name, key)
            metadata = self.metadata_manager.get_bucket_metadata(bucket_name, key)[
                "versions"
            ][latest_version]
    ...
    async def put_object_legal_hold(
        self, bucket, key, legal_hold_status, version_id=None, is_sync=True
    ):
        try:
    ...
    async def get_object_legal_hold(self, bucket, key, version_id=None, is_async=True):
        try:
            ...
            if version_id not in metadata["versions"]:
                raise KeyError(
                    f"Version id '{version_id}' not found for object key '{key}' in metadata"
                )

            legal_hold = metadata["versions"][version_id].get(
                "LegalHold", {"Status": "OFF"}
            )
    ...
    async def get_object_retention(self, bucket, key, version_id=None, is_sync=True):
        try:
            ...
            retention = metadata["versions"][version_id].get(
    ...
    async def put_object_retention(
        ...
        try:
            if retention_mode not in ["GOVERNANCE", "COMPLIANCE"]:
                raise ValueError(
                    "Retention mode must be either 'GOVERNANCE' or 'COMPLIANCE'"
                )
            ...
            try:
                ...
                raise ValueError(
                    "Retain until date must be a valid date in the format YYYY-MM-DDTHH:MM:SSZ"
                )
    ...
    async def put_object_lock_configuration(
        ...
        try:
            ...
            if object_lock_enabled not in ["Enabled", "Disabled"]:
                raise ValueError(
                    "Object lock enabled must be either 'Enabled' or 'Disabled'"
                )
            if days < 0:
...
async def main():
    ...
    try:
        torrent_info = await s3_client.get_object_torrent(
            "bucket1", "file.txt", version_id="1"
        )
    ...
    try:
        response = await s3_client.put_object_acl(
            "bucket1",
            "file2.txt",
            {"owner": "default_owner", "permissions": ["READ", "WRITE"]},
            version_id="3",
        )
    ...
    try:
        ...
        await s3_client.put_object_tagging(
            "bucket1", "file.txt", new_tags, version_id=1
        )
    ...
    client = S3ClientSimulator(
        "C:/Users/user1/Desktop/server/metadata.json", "C:/Users/user1/Desktop/server"
    )

    # Example for copy_object
    copy_result = await client.copy_object(
        "bucket2", {"Bucket": "bucket1", "Key": "object1.txt"}, "QQQQQQQ.txt", False
    )
    print(copy_result)
    ...
    delete_objects_result = await client.delete_objects(
        "bucket2", {"Objects": [{"Key": "kkk.txt"}]}
    )
